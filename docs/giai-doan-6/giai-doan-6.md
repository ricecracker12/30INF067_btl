...
**Danh sách `action`** (hằng trong `Moderation.Domain.AuditActions`, test canh không trùng):
...
CREATE TABLE moderation.audit_logs (
    id          bigserial    PRIMARY KEY,                               -- PTTK: bigserial; khóa keyset của GET /admin/audit-logs
