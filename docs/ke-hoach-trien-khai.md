...
## 0C. Nhánh FRONTEND chạy song song (từ GĐ2) — Next.js 14

> Kế hoạch gốc nhắc Next.js 14 ở tech stack nhưng **không giai đoạn nào giao việc làm frontend**.
> Mục này lấp lỗ hổng đó.

**Phân bổ nhân lực:** GĐ1 cả 3 người làm backend. **Từ GĐ2 trở đi: 1 người tách sang frontend,
2 người ở lại backend.**
...
**Quy tắc vận hành — FE trễ BE đúng một nhịp:** frontend luôn dựng slice mà backend **vừa hoàn
thành và đã đóng băng hợp đồng**, không bao giờ bám vào API đang viết dở. API dở thì Swagger đổi
mỗi giờ, sửa theo còn tốn thời gian hơn là chờ một nhịp.

| Ngày | Backend (2 người) | Frontend (1 người) |
|---|---|---|
| 3–5 · GĐ1 | Auth | — *(cả 3 làm backend)* |
| 5–8 · GĐ2 | Profile + Content | **Auth UI (GĐ1)** + app shell + interceptor 401→refresh |
| 8–12 · GĐ4 | Social graph + Feed | Đăng bài + upload ảnh *(GĐ2)* ← **lộ lỗi CORS R2 tại đây** |
| 12–14 · GĐ3 | Comment + Reaction | News feed + cursor *(GĐ4)* |
| 14–18 · GĐ5 | Chat realtime | Comment + reaction *(GĐ3)* |
| 18–20 · GĐ6 | Notification + Search + Mod | **Chat UI (GĐ5)** |
| 20–22 · GĐ7 | Production hardening | Search + admin *(GĐ6)* + hoàn thiện UI/UX + deploy FE |

**Hai trang HTML tối giản — công cụ đo, không phải sản phẩm** (xóa khi FE thật thay thế):

| Khi nào | Trang gì | Vì sao cần |
|---|---|---|
| **GĐ1** (~1 giờ) | Đăng ký → đăng nhập → `GET /me` → ép 401 → refresh → gọi lại | Kiểm chứng thiết kế thu hồi token (Mục 7.5) **trong trình duyệt thật**: cookie, CORS, preflight — những thứ integration test không chạm tới |
| **GĐ5** (~1 giờ) | 2 SignalR JS client cạnh nhau, đo p95 gửi→nhận | **Chốt GOAL-02 đúng hạn GĐ5.** Theo quy tắc trễ một nhịp thì chat UI thật mãi GĐ6 mới có — trang này gỡ mâu thuẫn đó mà không bắt người FE phá nhịp |

**Việc backend phải giao trước, nếu không nhánh FE đứng bánh:**
- **CORS — làm ở GĐ1, không phải GĐ2.** Người làm FE bắt đầu ngày đầu GĐ2; để CORS sang GĐ2 là
  họ ngồi chờ. Chi phí vài dòng.
...
**Hợp đồng giữa hai nhánh:** frontend bám vào **Swagger JSON trên staging**
(`/swagger/v1/swagger.json`) làm nguồn sự thật duy nhất. Backend đổi hợp đồng phải báo trước —
đây là lý do quy tắc "đóng băng hợp đồng API" ở mỗi giai đoạn trở thành bắt buộc, không còn là
lời khuyên.

> ⚠️ **Rủi ro đã biết của phương án này:** GĐ4 (feed, trọng điểm hiệu năng) và GĐ5 (realtime,
> trọng điểm kỹ thuật) sẽ chỉ còn **2 người** làm backend. Nếu thấy trễ ở GĐ4, phương án ứng phó
> là kéo người frontend về hỗ trợ và chấp nhận UI dừng ở mức mỏng cho tới GĐ7.

---

## Đường lõi & thứ tự ưu tiên

**Đường lõi** — luồng phải chạy được end-to-end trước mọi thứ khác, đóng lại ở **~Ngày 12**:
...
**Thứ tự ưu tiên khi vỡ tiến độ** — có bảng này thì lúc trễ ở Ngày 18 biết cắt gì ngay, không phải họp:
...
> Từ GĐ2, mỗi giai đoạn còn có nhánh frontend chạy song song — xem Mục 0C.
...
- **Làm như nào:**
  ...
  - **Đóng băng hợp đồng API auth trước cuối Ngày 4** — GĐ2 khởi động Ngày 5, không thể chờ.
...
- **Làm như nào:**
  ...
  - **Dùng lại nguyên xi hạ tầng AuthZ của GĐ1** — đây là module đầu tiên tiêu thụ nó, làm chuẩn
    ...
    cách kiểm tra riêng.
  - **Nhánh frontend khởi động ở đây** (Mục 0C): 1 người tách sang Next.js. Backend phải giao
    trước 3 thứ để không chặn họ — **CORS cho origin frontend**, Swagger JSON ổn định trên staging,
    và endpoint xin pre-signed URL.
- **Kiểm tra:** AC US-004: AC-01 đăng công khai → 201; AC-02 bài rỗng không ảnh → 400 (BR-01);
  ...
  **Upload ảnh thật từ trình duyệt lên R2** — bước duy nhất chứng minh CORS đúng (ISS-02);
  interceptor 401→refresh chạy được trong trình duyệt (kiểm chứng `giai-doan-1.md` Mục 7.5).
...
### GĐ 4 — Social Graph + News Feed: UC-10/11, UC-13, UC-08 (Ngày 8–12) ⚠️ trọng điểm hiệu năng
...
### GĐ 3 — Tương tác: bình luận 3 cấp + cảm xúc: UC-06, UC-07 (Ngày 12–14)
...
- **Kiểm tra:** AC US-015 (AC-01 B online nhận ≤1s + đủ trạng thái; AC-02 B offline → badge khi online;
  ...
  **E2E 2 trình duyệt đo p95 gửi→nhận ≤ 1s** (NFR-PERF-03, GOAL-02) — đo bằng **trang HTML tối
  giản với 2 SignalR JS client** (~1 giờ, Mục 0C), vì theo quy tắc FE trễ một nhịp thì chat UI thật
  mãi GĐ6 mới có. Trang này là công cụ đo, xóa khi chat UI thật thay thế.
...
**Nhánh frontend (Mục 0C)** chạy song song từ GĐ2, luôn bám sau backend một nhịp: chỉ dựng màn
hình khi API của giai đoạn đó đã đóng băng hợp đồng. Không đảo được hai mốc: **CORS phải xong đầu
GĐ2** (nếu không frontend không khởi động được) và **chat UI phải xong trong GĐ5** (nếu không
GOAL-02 không có cách nghiệm thu).
