...
- **`.claude/rules/`** — luật **bắt buộc** khi làm việc trên repo, đọc trước khi làm việc tương ứng.
  ...
  - `frontend-rules.md` — luật lane FE trong `src/frontend/`: bốn tầng `app/ → features/ → components/ + lib/`
    ...
    `Đ-E1`–`Đ-E18` (Đ-E15: CSP có nonce; Đ-E16: SignalR xác thực bằng vé ngắn hạn; Đ-E18: hub dev nối thẳng API, CSP chỉ nới ở dev); chi tiết ở hướng dẫn khối E.
...
| Resource | Use for |
| --- | --- |
| `gitnexus://repo/30INF067_btl/context` | Codebase overview, check index freshness |
| `gitnexus://repo/30INF067_btl/clusters` | All functional areas |
| `gitnexus://repo/30INF067_btl/processes` | All execution flows |
| `gitnexus://repo/30INF067_btl/process/{name}` | Step-by-step execution trace |
