...
public sealed class PostResponseMapperTests
{
    ...
    public void moderation_chi_co_khi_bai_an_va_nguoi_doc_la_tac_gia()
    {
        ...
        Assert.Equal(new PostModeration("hidden", "violence", hiddenAt), Mapper.ToResponse(hidden, [], null, null, author).Moderation);
        Assert.Null(Mapper.ToResponse(hidden, [], null, null, Guid.NewGuid()).Moderation);
        Assert.Null(Mapper.ToResponse(PostOf(author), [], null, null, author).Moderation);
    }
    ...
    public void Bai_an_thieu_ly_do_thi_reasonCode_la_other()
    {
        ...
        Assert.Equal(PostResponseMapper.UnknownReasonCode, Mapper.ToResponse(hidden, [], null, null, author).Moderation!.ReasonCode);
    }
}
